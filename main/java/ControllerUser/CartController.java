...
public class CartController extends BaseController {
	...
	public String EditCart(HttpServletRequest request, HttpSession session, @PathVariable long id,
		...
		session.setAttribute("TotalSalePrice", cartservice.TotalPrice(cart));
	...
	public String checkoutBill(HttpSession session, @ModelAttribute("bills") Bills bill) {
		...
		Double totalSalePrice = 0.0;
		 ...
		 Long userId = (Long) session.getAttribute("userId");
			if (userId != null) {
		        // Gán ID người dùng vào hóa đơn
		        bill.setId_client(userId);
		    } 
	    
	    System.out.println("id ng dung là" + userId);
		...
		if (carts != null) {
			for (Map.Entry<Long, CartDTO> entry : carts.entrySet()) {
				...
				int productSalePercentage = cartDTO.getProduct().getSale(); 
		...
		if (billService.AddBills(bill) > 0) {
			...
			for (Map.Entry<Long, CartDTO> entry : carts.entrySet()) {
				...
				long productSizeId = _homService.cartDao.findProductSizeId(productId, size);

				// Cập nhật số lượng kho cho sản phẩm theo kích cỡ
				_homService.cartDao.updateStock(productSizeId, quantityInCart);
			}
		}
